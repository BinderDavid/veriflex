...
This file defines the `nullable` predicate which checks whether a regular expression matches the empty string and the `derivative` function which computes the Brzozowksi derivative.
Both of these functions are proven correct relative to a specification in terms of the `Matching` relation.
-/
